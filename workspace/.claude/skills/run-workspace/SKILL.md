...
pip3.11 install onnxruntime   # if not present — ultralytics, face_recognition, cv2 already installed
```
...
53 tests in 4 files covering FrameQualityChecker, DiversityFilter, VideoFrameExtractor, PersonManager CRUD, and the full collection integration pipeline.
...
import config
from core.frame_extractor import FrameQualityChecker, FrameDiversityFilter, VideoFrameExtractor
...
python3.11 training_manager.py

# Real-time recognition (opens camera window, press q to quit)
python3.11 recognizer.py --camera 0 --tolerance 0.6

# Augment dataset (no-encode to skip pkl rebuild)
python3.11 augment_dataset.py --person Khai --no-preview --no-encode
...
**Python version: use `python3.11`, not `python3`**. The system `python3` is 3.13 and has none of the required packages (`face_recognition`, `ultralytics`, `onnxruntime`). All packages are installed under Python 3.11.
...
**YOLO `.onnx` model not present by default**. `config.MODEL_PATH` falls back to `yolov11n-face.pt`. To create the ONNX version: `cd src && python3.11 export_onnx.py`. The `.pt` fallback works fine.

## Troubleshooting

| Error | Fix |
|---|---|
| `ModuleNotFoundError: No module named 'face_recognition'` | Run with `python3.11`, not `python3` |
| `ModuleNotFoundError: No module named 'onnxruntime'` | `pip3.11 install onnxruntime` |
